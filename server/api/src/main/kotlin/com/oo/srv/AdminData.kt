...
val routersList = ClassLoader.getSystemResourceAsStream("routersList.json").use {readInputStreamAsString(it!!)}
...
val roleRouters = ClassLoader.getSystemResourceAsStream("roleRouters.json").use {readInputStreamAsString(it!!)}
...
class TestInitRunner(
    ...
    ,@Resource val transRepo:BizTranRepository
...
val tokens = ClassLoader.getSystemResourceAsStream("tokens.json")
    .use { readInputStreamAsString(it!!) }
    .let { Gson().fromJson(it,LinkedTreeMap::class.java) } as Map<String,*>
val articleList = ClassLoader.getSystemResourceAsStream("articleList.json")
    .use { readInputStreamAsString(it!!) }
